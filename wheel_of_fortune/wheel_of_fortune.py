...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        pos_letters = self.getPossibleLetters(guessed)
        if pos_letters == []:
            return 'pass'
        if self.smartCoinFllip:
            return [l for l in pos_letters if l in self.SORTED_FREQUENCIES][-1]
        else:
            return random.choice(pos_letters)
